---
title: "Improve random forest performance using variable importance in R"
...
# tltr
I fine-tune a random forest (RF) model using variable importance in R.
I show how to use `DALEX` package to understand a random forest built with `ranger` package, simplify it, improve performance and run time. I use `recipes` package to handel categorical variables, missing values and novel levels.
...
I take an example from [fast.ai](https://www.fast.ai/) Machine Learning course in Python and show how to do similar analysis using tools available in R.
I use Kaggle's competition [Blue Book for Bulldozers](https://www.kaggle.com/c/bluebook-for-bulldozers/data) data and predict an auction price for a heavy equipment with a random forest. 
...
After I have the model I plug it into `DALEX` explainer, perform variable importance shuffling and visualize the results. Next by interpreting the results I make adjustments to the model.

The main idea here is to build a RF model as soon as possible without thinking too much about the data and use model interpretation techniques to make it better.
...
```{r message = FALSE, warning = FALSE}
data_raw <- read_csv(file = "Train.csv")
```

### Data pre-processing

There are few things to be done with the data before pluging them into the model.
* I need to handel missing data both in numeric and cardinal columns. In the numeric columns missing values are replaced with median with a `recipe` step `step_medianimpute()`. I add a dummy column for every numeric column that contains NAs using `add_dummy_for_na_numeric()` that I define myself.
I replace NAs in character columns with "NA" string and threat missing values as a separate level.
* I make sure my cardinal variables enter the model as factors. I use `recipe` `step_novel()` to make sure model can handel unseen levels in the validation set.
* I add new features based on sale date column such as: new features: elapsed number of days since 1 Jan 1970, day, week, month, year, day of week, day of year and indicators is year/quarter/month beginning/end using `proc_date()`.
...
Split data into training and validation set using `initial_time_split()` function. I keep 80% of data in the training set and 20% in the validation.
...
valid_small <- testing(data_step1_split)
```
...
* One-hot-encoding character columns with 7 or less levels with `step_dummy()`
...
```{r}
vars_below_8levels <- data_step1 %>% select_if(~is.character(.) & n_distinct(.) < 8)  %>% names()
```
...
recipe_steps <- recipe_formula %>%
  ...
  step_dummy(vars_below_8levels, one_hot = T)
...
```{r}
train_set <- recipe_steps_on_training %>%
  juice()

y_train <- train_set$log_SalePrice
x_train <- train_set %>% select(-log_SalePrice)


valid_set <- recipe_steps_on_training %>%
  bake(valid_small)

x_valid <- valid_set %>% select(-log_SalePrice)
y_valid <- valid_set$log_SalePrice
```
...
So next I select the features before the curve flattens almost completely at around 0.135 mean dropout loss and build a model on the remaining features subset.

```{r}
var_to_keep <-  summarize_vi %>%
  filter(mean_dropout_loss > 0.155 & variable != "_baseline_") %>% 
  select(variable)
...
data_step1 %>% filter(MachineID == 2283592)
```
SalesID is a unique identifier that sorts sales from the oldest to the newest.
Kaggle's description says MachineID is unique machine identifier and that machine can be sold multiple times. I expect that RF model is overfitting on these columns so I remove them from the dataset and see how it impacts the model.
...
It seems cleaning YearMade and dropping IDs improved the model. 

## Conclusions

I manage to improve model's RMSE on validation set from 0.28 to 0.25, reduce number of features and get first inisghts into which variables matter to the model.
